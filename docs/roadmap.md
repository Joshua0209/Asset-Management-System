...
| Security CI gates (full) | Wed–Thu | ✅ Done (PR [#58](https://github.com/Joshua0209/Asset-Management-System/pull/58)). SonarQube already shipped W1; the branch added `pip-audit` (Python SCA), `npm audit --omit=dev --audit-level=high`, and `dependency-check --failOnCVSS 7`. Optional `NVD_API_KEY` secret bumps NVD rate limit. *(Dependency-Check + `NVD_API_KEY` were later replaced by Trivy — see `infra/aws/tasks/README.md`.)* |
