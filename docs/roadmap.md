...
| Task | Target | Notes |
|------|--------|-------|
| Monorepo setup | Tue–Wed | Python (FastAPI) backend + React (Vite) frontend. Use a simple workspace layout (e.g., `backend/` + `frontend/`). Shared API contract via OpenAPI schema auto-generated by FastAPI |
...
| Task | Target | Notes |
|------|--------|-------|
| React + Vite project in monorepo | Tue–Wed | TypeScript strict mode, `react-router-dom` for routing |
| UI library setup (Ant Design / shadcn) | Wed | Pick one; establish component patterns |
| i18n framework (`react-i18next`) | Wed–Thu | zh-TW + en. Set up early — retrofitting is painful |
