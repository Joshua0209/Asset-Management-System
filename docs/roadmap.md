...
**Status (2026-05-25):** W1–W5 done. **W6 code work landed on `main`; buffer + rehearsals start Mon May 26 with the rolling-deploy block still open.** The W6 work that was mid-pivot on 2026-05-24 has now merged. PR [#89](https://github.com/Joshua0209/Asset-Management-System/pull/89) squash-merged the full `feat/observability` integration branch carrying Phases 1, 2, 4, 5, 8 of the original implementation plan plus Phases 2–4 of the [production-migration plan](docs/plans/observability-prod-migration-plan.md), so the backend is now OTLP-native (no `prometheus_client`, no `/metrics` route, OTel SDK Counter/Tracer/Logger pushes direct to Grafana Cloud over gRPC) and ECS task definitions point at the GC OTLP gateway with a cross-account IAM role for GC's hosted CloudWatch integration. Phase 5 k6 load surface plus a per-request structured access log (rides the OTel logging bridge so every line carries `trace_id`/`span_id`) shipped via PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85). The W5 DESIGN.md theme carry-over closed via PR [#80](https://github.com/Joshua0209/Asset-Management-System/pull/80) (design tokens wired through Antd `ConfigProvider`, demo-visible surfaces audited against the four-pillar checklist). PRs [#83](https://github.com/Joshua0209/Asset-Management-System/pull/83) and [#90](https://github.com/Joshua0209/Asset-Management-System/pull/90) align the backend task-def around a single `DATABASE_URL` secret, set `FORWARDED_ALLOW_IPS=*` for ALB-fronted client IPs, add an automated pre-deploy one-off ECS task that runs `alembic upgrade head`, and fix the migrate/seed CI ordering so one-off task defs register before `run-task`. **The post-merge CD rolling deploy has not completed cleanly, so the production stack is not yet correctly deployed**; until it is, the k6 sustained-QPS run and the Grafana Cloud dashboard verification both wait on it. Realistic demo seed (60 active + 3 disposed assets, 12 repair requests spanning every FSM state, zh-TW user/department/vendor names) landed via PR [#87](https://github.com/Joshua0209/Asset-Management-System/pull/87); expanded unit test coverage (backend 484 tests at 97% coverage, frontend 184 tests) landed via PR [#88](https://github.com/Joshua0209/Asset-Management-System/pull/88). CI itself got faster via PR [#76](https://github.com/Joshua0209/Asset-Management-System/pull/76) (parallel backend lint/typecheck/test jobs, path-filtered runs, Trivy in place of OWASP Dependency-Check) and PR [#81](https://github.com/Joshua0209/Asset-Management-System/pull/81) (test+coverage jobs always run so SonarQube never skips). **Open for buffer week:** unblock the rolling deploy, then run the k6 load test and verify the GC dashboards against real prod telemetry; Playwright E2E for the 6 critical flows; Phase 6 of the obs plan (delete repo-side dashboard JSONs once GC import is verified); slides + report draft; May 26 + May 29 rehearsals.
...
| AWS setup: ECS Fargate + ALB + RDS Multi-AZ | Tue–Thu | ✅ Done (PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63), merged 2026-05-20 W6 Tue). Architecture pivoted from "EC2 ×2 + manual orchestration" to ECS Fargate. Task definitions in `infra/ecs/` use `__NAME__` sentinel placeholders (no env-var collisions), substituted at deploy time via the new `.github/actions/render-task-def/` composite action with escape-safe sed (`\`, `|`, `&` neutralised) and a post-substitution guard. Operator setup: dedicated `ams/prod/app` Secrets Manager entry for `JWT_SECRET` + `BOOTSTRAP_MANAGER_PASSWORD`, RDS secret `ams/prod/rds` (both pinned to `AWSCURRENT`), three IAM roles (`ams-ecs-task-execution`, `ams-backend-task`, `ams-frontend-task`), GitHub OIDC trust on `StringEquals`, identity policy with `iam:PassRole` scoped via `iam:PassedToService=ecs-tasks.amazonaws.com`. RDS (`ams-database`) + S3 (`ams-repair-images-prod`) + ECR live in `ap-east-2`; `ams-backend` / `ams-frontend` services in the `ams-prod` cluster. **Operator-side follow-up:** trigger a `workflow_dispatch` to confirm both task defs render correctly against the real GitHub secrets/variables |
...
- [ ] App running on AWS (accessible via public URL) — task-def, secrets, IAM, and pre-deploy `alembic upgrade head` automation are all merged (PRs [#83](https://github.com/Joshua0209/Asset-Management-System/pull/83), [#90](https://github.com/Joshua0209/Asset-Management-System/pull/90)), but the post-merge CD rolling deploy has not completed cleanly. **Carries to buffer week**
- [x] CI/CD: push to main triggers the deploy pipeline (the rolling deploy itself is still failing; investigation carries to buffer week)
- [ ] Zero-downtime deploy demonstrated (deploy during load test) — pairs with the buffer-week k6 run, both gated on the rolling deploy unblocking
...
- [ ] E2E: 6 flows passing — **carries to buffer week**
...
### Current state (2026-05-25): Grafana Cloud is the single backend, on `main`, with prod deploy live

The W6 work landed in two phases. The first half built the self-hosted Grafana stack described in the historical narrative below (Phases 1–5 of `docs/plans/observability-implementation-plan.md`). The second half, triggered by seven decisions locked on 2026-05-24, replaced it with a hosted Grafana Cloud stack named `ams` and removed every observability container from local dev. Both halves are now merged to `main` via PR [#89](https://github.com/Joshua0209/Asset-Management-System/pull/89) (the squash-merge of `feat/observability`) plus PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85) (Phase 5 k6 + access log on the post-Phase-3 base). The full cutover plan is in [docs/plans/observability-prod-migration-plan.md](docs/plans/observability-prod-migration-plan.md). The new shape:
...
- [x] Six dashboards authored against the OTLP metric model + GC CloudWatch panels for ECS/RDS/ALB (PR [#78](https://github.com/Joshua0209/Asset-Management-System/pull/78), dashboard hygiene fixes in PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85)). **In-prod verification (sync + panel render against real telemetry) is gated on the rolling deploy.**
- [x] Phase 5 k6 load surface plus per-VU JWT cache, six scenario scripts, container-run support via `host.docker.internal`, and `K6_PROMETHEUS_RW_*` for remote-write into GC Prom (PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85)). **Sustained-QPS run against the deployed ALB is gated on the rolling deploy.**
...
| Infra / Observability | 2 | Instrument backend, stand up the Grafana stack locally, provision dashboards, run the post-PR-[#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) `workflow_dispatch` AWS deploy smoke test |
...
| **AWS provisioning carry-over** | Mon–Tue | ✅ PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) merged 2026-05-20 (operator hydration of task-def placeholders + `ams/prod/app` secret + IAM roles, hardened against unset/empty vars and sed-metachar values, AWSCURRENT pinning, ECR image-scan gate at CVSS ≥ 7). **Open:** `workflow_dispatch` smoke test against the real GitHub secrets/variables, then confirm push-to-main triggers an ECS rolling deploy that reaches a steady state with ALB target-group health checks green on `/ready` |
...
| **Small frontend consistency PR** | Mon | ✅ PR [#65](https://github.com/Joshua0209/Asset-Management-System/pull/65) merged 2026-05-20: "Fault Content" → "Fault Description" in manager modals, Reviews list shows Request ID as the first column to mirror the holder list, `formatDateTime` respects `i18n.resolvedLanguage` so dates render `下午 8:00` in zh-TW and `8:00 PM` in en, `RepairRequestList` + `Reviews` share rendering helpers for request ID / asset / status |
...
> **Note:** Several checklist items below were rewritten by the 2026-05-24 Grafana Cloud pivot (e.g. the `mysqld-exporter`, the compose overlay, the in-repo CloudWatch datasource block). The current-state checklist under "Current state (2026-05-25)" above is the authoritative tracker. The list here is preserved against the original W6 scope.

- [x] DESIGN.md theme applied (PR [#80](https://github.com/Joshua0209/Asset-Management-System/pull/80))
- [x] AWS provisioning code merged (PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) merged 2026-05-20)
- [ ] App reachable on AWS: task-def, secrets, IAM, and pre-deploy Alembic migration as a one-off ECS task are merged (PRs [#83](https://github.com/Joshua0209/Asset-Management-System/pull/83), [#90](https://github.com/Joshua0209/Asset-Management-System/pull/90)) but the rolling deploy has not completed cleanly. Carries to buffer week
- [x] Backend instrumented: OTLP traces, OTel metrics, OTel-bridged structured JSON logs (incl. per-request access log from PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85)), Pyroscope profiles enabled in prod (PR [#89](https://github.com/Joshua0209/Asset-Management-System/pull/89) reversed the original "off in prod" decision)
- [x] Backend prod task definition runs `gunicorn --workers 1` (ECS scales via task count, single `/metrics` per task aligned with the OTel SDK push model)
- [x] Frontend instrumented: browser OTLP for the asset-list → repair-detail click path (PR [#72](https://github.com/Joshua0209/Asset-Management-System/pull/72))
- [N/A] ~~`mysqld-exporter` sidecar~~ — removed by the GC pivot. MySQL server-side metrics now come from GC's hosted RDS CloudWatch integration via the cross-account IAM role (PR [#89](https://github.com/Joshua0209/Asset-Management-System/pull/89))
- [N/A] ~~CloudWatch Grafana datasource provisioned in repo~~ — replaced by GC's hosted CloudWatch integration (Phase 4)
- [N/A] ~~Grafana stack runs via `docker compose -f docker-compose.yml -f docker-compose.observability.yml up`~~ — overlay deleted in Phase 2 of the migration plan. Local `docker compose up` brings up only `mysql + backend + frontend`; telemetry pushes direct to GC when credentials are supplied
- [x] Six dashboards provisioned in repo (Operations Overview, Service Drilldown, Repair Journey, Logs/Traces/Profiles correlation, MySQL, Start Here) (PR [#78](https://github.com/Joshua0209/Asset-Management-System/pull/78))
- [ ] One end-to-end correlation demo verified live in GC: click a slow request in a dashboard, jump to the Loki log line, jump to the Tempo trace, jump to a Pyroscope flamegraph for the same window
- [x] k6 load surface shipped (PR [#85](https://github.com/Joshua0209/Asset-Management-System/pull/85)). **Sustained peak QPS for 10 min + Grafana screenshots for slides are gated on the rolling deploy unblocking**
...
- [x] Test coverage ≥ 80%: backend 484 tests at 97% coverage (`pytest --cov=app`), frontend 184 tests (PR [#88](https://github.com/Joshua0209/Asset-Management-System/pull/88)). SonarQube always-on after PR [#81](https://github.com/Joshua0209/Asset-Management-System/pull/81)
...
| **AWS rolling deploy not converging** | Active | High | All deploy-side code is merged (PRs [#83](https://github.com/Joshua0209/Asset-Management-System/pull/83), [#90](https://github.com/Joshua0209/Asset-Management-System/pull/90)): single `DATABASE_URL` secret, `FORWARDED_ALLOW_IPS=*` for ALB-fronted client IPs, automated pre-deploy one-off ECS task running `alembic upgrade head`, one-off task defs registered before `run-task`. **But the post-merge CD rolling deploy has not completed cleanly**, so the live demo, the k6 sustained-QPS run, and the GC dashboard in-prod verification are all blocked. **Mitigation:** treat unblocking the deploy as the buffer-week top priority. If it cannot be resolved before Jun 2, the live demo falls back to `docker compose up` against Grafana Cloud from the laptop, and the slides cover production deployment via the architecture diagram and CD pipeline screenshots |
...
| **Playwright E2E suite still not authored as buffer week begins** | Active | Medium | Six critical flows (login, holder submit repair, manager approve, manager complete, asset search, register asset) need Playwright coverage before the May 26 rehearsal lands cleanly. **Mitigation:** start Mon May 26 with the QA seat freed from W6 obs work. Two flows per day across Mon–Wed leaves slack for CI wiring on Thu, with Fri/weekend buffer before the Jun 2 presentation. Fall back to the recorded backup demo video if any flow regresses live |
