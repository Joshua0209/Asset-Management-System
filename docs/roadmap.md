...
- [ ] App running on AWS (accessible via public URL) — pending `workflow_dispatch` smoke test against real GitHub secrets/variables (PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) follow-up)
- [x] CI/CD: push to main auto-deploys — code path live after PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) merge; first real deploy verification is the W6 follow-up
...
| **AWS provisioning may not converge by demo** | Active (downgraded) | Medium | PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) merged 2026-05-20 with operator hydration + hardened placeholder substitution + AWSCURRENT pinning + identity-policy snippet. What remains is the `workflow_dispatch` smoke test — empty/unset `vars.*` now fail fast at the workflow level (PR [#63](https://github.com/Joshua0209/Asset-Management-System/pull/63) `require_var`), so a misconfigured deploy aborts before reaching ECS instead of shipping a broken task def. **Mitigation:** demo remains reproducible on `docker compose up` if the first rolling deploy needs another iteration — the observability stack is local-first by design. If AWS slips, the slides cover deployment via the architecture diagram and the live demo runs on localhost |
