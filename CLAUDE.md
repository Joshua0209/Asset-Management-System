...
- CI (`.github/workflows/ci.yml`) is path-filtered. A `changes` job (dorny/paths-filter) emits `backend`/`frontend` booleans that downstream jobs gate on. Backend pipeline is split into three parallel jobs: `backend-lint` (ruff), `backend-typecheck` (mypy --strict), `backend-test` (pytest + coverage). Frontend stays as a single job. Security baseline (`secrets` via gitleaks, `sast` via Semgrep OWASP top-10) runs unconditionally on every PR + main push + workflow_dispatch — they are NOT path-filtered, because the deploy gate treats `skipped` as not-failure. SCA jobs `pip-audit` (Python, HIGH+ in prod deps), `npm-audit` (Node, HIGH+ in prod deps), and `trivy` (filesystem scan, HIGH+CRITICAL, NO `ignore-unfixed`) ARE path-filtered, because the lockfiles that could introduce a CVE live under `backend/**` / `frontend/**`. `sonarqube` runs only when BOTH `backend-test` and `frontend` succeeded — partial-coverage uploads would silently distort project-wide coverage trends. A red gate from any of these usually means a real issue — do not bypass. The CVE gates respect `--audit-level=high` / Trivy `severity: HIGH,CRITICAL` on purpose: MEDIUM/LOW noise would dilute the signal.
- Deploy jobs live in `.github/workflows/ci.yml` and fire on push to `main` (with at least one path-filter side flipped) or `workflow_dispatch` against `refs/heads/main` after the quality/security gates pass. `deploy-backend` and `deploy-frontend` are individually gated on `needs.changes.outputs.<side>` so a frontend-only commit does not roll the backend ECS service. They build prod images from `Dockerfile.prod` (backend + frontend) via `build-and-push`, push to ECR via OIDC role assumption (no long-lived AWS keys), then render `infra/aws/tasks/*-task-def.json` and run ECS rolling updates with `wait-for-service-stability`. Required secret: `AWS_DEPLOY_ROLE_ARN`. Required vars: `AWS_REGION`, `ECR_REPOSITORY_BACKEND/FRONTEND`, `ECS_CLUSTER`, `ECS_SERVICE_BACKEND/FRONTEND`. See `infra/aws/tasks/README.md` for the IAM trust policy snippet.
