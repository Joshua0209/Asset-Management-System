...
- Deploy jobs live in `.github/workflows/ci.yml` and fire on push to `main` or manual dispatch after the quality/security gates pass. They build prod images from `Dockerfile.prod` (backend + frontend), push to ECR via OIDC role assumption (no long-lived AWS keys), then render `infra/ecs/*-task-def.json` and run ECS rolling updates with `wait-for-service-stability`. Required secret: `AWS_DEPLOY_ROLE_ARN`. Required vars: `AWS_REGION`, `ECR_REPOSITORY_BACKEND/FRONTEND`, `ECS_CLUSTER`, `ECS_SERVICE_BACKEND/FRONTEND`. See `infra/ecs/README.md` for the IAM trust policy snippet.
