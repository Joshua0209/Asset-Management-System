...
- Deploy jobs live in `.github/workflows/ci.yml` and fire on push to `main` (with at least one path-filter side flipped) or `workflow_dispatch` against `refs/heads/main` after the quality/security gates pass. `deploy-backend` and `deploy-frontend` are individually gated on `needs.changes.outputs.<side>` so a frontend-only commit does not roll the backend ECS service. They build prod images from `Dockerfile.prod` (backend + frontend) via `build-and-push`, push to ECR via OIDC role assumption (no long-lived AWS keys), then render `infra/ecs/*-task-def.json` and run ECS rolling updates with `wait-for-service-stability`. Required secret: `AWS_DEPLOY_ROLE_ARN`. Required vars: `AWS_REGION`, `ECR_REPOSITORY_BACKEND/FRONTEND`, `ECS_CLUSTER`, `ECS_SERVICE_BACKEND/FRONTEND`. See `infra/ecs/README.md` for the IAM trust policy snippet.
