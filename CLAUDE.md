...
- The quality gate lives in a reusable workflow, `.github/workflows/ci-quality.yml`; `.github/workflows/ci.yml` is a thin caller that runs it on PRs and `.github/workflows/cd.yml` runs the same reusable workflow on pushes to `main`. It is path-filtered: a `changes` job (dorny/paths-filter) emits `backend`/`frontend`/`dashboards` booleans that downstream jobs gate on. Note the dorny filters key off `.github/workflows/ci.yml` and `.github/workflows/ci-quality.yml`, so edits to either workflow trigger the full pipeline. Backend pipeline is split into parallel jobs: `backend-lint` (ruff), `backend-typecheck` (mypy --strict), `backend-test` (pytest + coverage, 3-way pytest-split shard merged by `backend-coverage-merge`). Frontend stays as a single job. Security baseline (`secrets` via gitleaks, `sast` via Semgrep OWASP top-10) runs unconditionally on every PR + main push + workflow_dispatch — they are NOT path-filtered, because the deploy gate treats `skipped` as not-failure. SCA jobs `pip-audit` (Python, HIGH+ in prod deps), `npm-audit` (Node, HIGH+ in prod deps), and `trivy` (filesystem scan, HIGH+CRITICAL, NO `ignore-unfixed`) ARE path-filtered, because the lockfiles that could introduce a CVE live under `backend/**` / `frontend/**`. `sonarqube` runs only when BOTH `backend-coverage-merge` and `frontend-test` succeeded — partial-coverage uploads would silently distort project-wide coverage trends. A red gate from any of these usually means a real issue — do not bypass. The CVE gates respect `--audit-level=high` / Trivy `severity: HIGH,CRITICAL` on purpose: MEDIUM/LOW noise would dilute the signal.
- Deploy jobs live in `.github/workflows/cd.yml` and fire on push to `main` (with at least one path-filter side flipped) or `workflow_dispatch` against `refs/heads/main` after the quality/security gates pass. `deploy-backend` and `deploy-frontend` are individually gated on `needs.quality.outputs.<side>` so a frontend-only commit does not roll the backend ECS service. They build prod images from `Dockerfile.prod` (backend + frontend) via `build-and-push`, push to ECR via OIDC role assumption (no long-lived AWS keys), then render `infra/aws/tasks/*-task-def.json` and run ECS rolling update with `wait-for-service-stability`. Required secret: `AWS_DEPLOY_ROLE_ARN`. Required vars: `AWS_REGION`, `ECR_REPOSITORY_BACKEND/FRONTEND`, `ECS_CLUSTER`, `ECS_SERVICE_BACKEND/FRONTEND`. See `infra/aws/tasks/README.md` for the IAM trust policy snippet.
...
- **`S3ImageStorage`** — production backend. Selected by `REPAIR_IMAGE_BACKEND=s3` + `REPAIR_S3_BUCKET=<name>` (optional `REPAIR_S3_PREFIX`). Boto3 is lazy-imported, so dev environments don't need it. The ECS task definition under `infra/aws/tasks/backend-task-def.json` enables this by default in production.
...
- **`GET /health`** — liveness only. Always returns `{"status": "ok"}`. A pure process-up signal; not wired to any deployment probe.
- **`GET /ready`** — readiness probe. Runs `SELECT 1` against the DB; returns 200 + `{"status":"ready"}` on success or 503 + `{"status":"not_ready"}` if SQLAlchemy raises. This is the probe everything actually hits: the ECS container health check (`infra/aws/tasks/backend-task-def.json`), the ALB target group, and the compose `backend.healthcheck` all probe `/ready`, so an unhealthy task is drained/replaced without killing an otherwise-fine container (e.g. during RDS Multi-AZ failover), and `depends_on: service_healthy` waits for actual DB connectivity, not just the process being up.
